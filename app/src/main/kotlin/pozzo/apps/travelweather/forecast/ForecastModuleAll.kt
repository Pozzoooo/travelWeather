...
class ForecastModuleAll : ForecastModule() {
    ...
    override fun forecastClients(retrofitBuilder: Retrofit.Builder, mapAnalytics: MapAnalytics):
        ...
        forecasts[Random.nextInt().and(Integer.MAX_VALUE) % 20/*5*/] = ForecastModuleDarkSky().forecastClient(retrofitBuilder)
        forecasts[Random.nextInt().and(Integer.MAX_VALUE) % 10/*100*/] = ForecastModuleWeatherUnlocked().forecastClient(retrofitBuilder, mapAnalytics)
        forecasts[Random.nextInt().and(Integer.MAX_VALUE) % 6/*60*/] = ForecastModuleOpenWeather().forecastClient(retrofitBuilder)
